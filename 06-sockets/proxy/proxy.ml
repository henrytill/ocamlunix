let get_regexp =
  { Url.regexp = Str.regexp "^[Gg][Ee][Tt][ \t]+\\(.*[^ \t]\\)[ \t]*\r"; fields = [ (1, None) ] }
...
let proxy_service (client_sock, _) =
  let service () =
    ...
    with End_of_file -> Url.error "Ill-formed request" "End_of_file encountered"
  in
...
let proxy () =
  let http_port =
    if Array.length Sys.argv > 1 then
      try int_of_string Sys.argv.(1) with Failure _ -> Url.error Sys.argv.(1) "Incorrect port"
    else
      try Unix.(getservbyname "http" "tcp").s_port
      with Not_found -> Url.error "http" "Unknown service"
  in
