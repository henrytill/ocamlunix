...
let write_header_to_buffer source infos kind =
  ...
  let put_path s offset =
    if String.length s <= 99
    then put 99 s offset
    else raise (Tarlib.Error ("path too long", s))
  in
  ...
  put_char
    ...
    156;
  let rec sum s i =
    if i < 0 then s else sum (s + Char.code (Bytes.get Tarlib.buffer i)) (pred i)
  in
...
let try_new_dir archive dir =
  try Hashtbl.find archive.dirfiles dir with
  | Not_found ->
...
let write_from archive file =
  if not (Filename.is_relative file) then raise (Tarlib.Error ("absolute path", file));
      ...
      | S_DIR when try_new_dir archive (st.st_ino, st.st_dev) ->
        ...
        Misc.iter_dir
          (fun file ->
            ...
            then ()
            else if file = Filename.parent_dir_name
            then ()
...
let _ =
  try Unix.handle_unix_error tar () with
  | Tarlib.Error (mes, s) ->
