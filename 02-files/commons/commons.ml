...
let output_char chan c =
  if chan.out_pos < Bytes.length chan.out_buffer
  then (
...
let output_string chan s =
  ...
  else if chan.out_pos = 0
  then ignore (Unix.write chan.out_fd (Bytes.of_string s) 0 len)
  else (
